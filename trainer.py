...
class Trainer:
    ...
    #for now just computes task covariance for validation trials of task k for only the trainable weights in the fully simple [without encoding and decoding layers] RNN
    # def task_cov(self, )
    ...
    # #to delete: lets
    # def update_cov_and_projection(self,weight_name=None, states=None):
    #     alpha = 1e-3
    #     if weight_name  == 'M_u.weight':
    #         input_states = self.test_etc['ins']
    #         output_states = self.test_etc['us']
    #     elif weight_name == 'reservoir.W_u.weight':
    #         input_states = self.test_etc['us']
    #         output_states = self.test_etc['xs']
    #     elif weight_name == 'reservoir.J.weight':
    #         with torch.no_grad():
    #             batch_unpacked_us = [] 
    #             batch_unpacked_xs = []
    #             for batch in range(self.test_etc['us'].shape[0]): 
    #                 batch_unpacked_us.append(self.test_etc['us'][batch])
    #                 batch_unpacked_xs.append(self.test_etc['xs'][batch])

    #             batch_unpacked_us = torch.cat(batch_unpacked_us, dim =1)
    #             batch_unpacked_xs = torch.cat(batch_unpacked_xs, dim=1)

                
    #             input_states = torch.cat((batch_unpacked_xs,batch_unpacked_us), dim=0)
            

    #     elif weight_name == 'reservoir.W_ro.weight':
    #         input_states = self.test_etc['xs']
    #         output_states = self.test_etc['vs']
    #     elif weight_name == 'M_ro.weight':
    #         input_states = self.test_etc['vs']
    #         output_states = self.test_etc['outs']
       
    #     input_activities = [] 
    #     output_activities = []
    #     with torch.no_grad():
            
    #         #covariance matrix update for reccurent weights
    #         if weight_name !=  'reservoir.J.weight':
                    
    #             for batch in range(input_states.shape[0]): 
    #                 input_activities.append(input_states[batch])
    #                 #need to concatenate
                
    #                 output_activities.append(output_states[batch])
                
    #             input_activities = torch.cat(input_activities, dim =1)
    #             output_activities = torch.cat(output_activities, dim =1)
                
    #             #calculate covariance matrix for current task
    #             input_activity_cov = input_activities @input_activities.T
    #             output_activity_cov = output_activities @ output_activities.T
    #             weight_block_matrix = self.net.state_dict(keep_vars=True)[weight_name]
    #             #update covariance matrix
    #             k = self.train_idx+1
    #             input_space_cov= ((k-1)*self.covs_and_projs[weight_name][0]+ input_activity_cov)/k
    #             output_space_cov = ((k-1)*self.covs_and_projs[weight_name][1]+ output_activity_cov)/k
    #         # covariance update for other parameters
    #         else:
    #             input_activities = input_states
    #             output_activities = []
    #             if self.args.D1 !=0:
    #                 weight_block_matrix_list = [self.net.reservoir.J.weight, self.net.reservoir.W_u.weight]
    #             else:
    #                 weight_block_matrix = [self.net.reservoir.J.weight, self.net.reservoir.M_u.weight]

    #             weight_block_matrix = torch.cat(weight_block_matrix_list, dim =1)
    #             print(f'block matrix {weight_block_matrix.shape}')
    #             for col in range(input_activities.shape[1]):
    #                 block_output = weight_block_matrix @ input_activities[:,col]
    #                 output_activities.append(block_output.reshape((block_output.shape[0],1)))
               
    #             output_activities = torch.cat(output_activities, dim =1)
    #             print(f'shape of output_activs {output_activities.shape}')
    #             input_activity_cov = input_activities @input_activities.T
    #             output_activity_cov = output_activities @ output_activities.T
    #             pdb.set_trace()
    #             k = self.train_idx +1
    ...
    #         self.covs_and_projs[weight_name][0], self.covs_and_projs[weight_name][1] = input_space_cov, output_space_cov
            
    #         P_in = torch.inverse(alpha**(-1) * input_space_cov + torch.eye(input_space_cov.shape[0]))
    #         if weight_name == 'reservoir.J.weight':
    #             P_out = torch.inverse(alpha**(-1)* weight_block_matrix @ input_space_cov @ weight_block_matrix.T +torch.eye(weight_block_matrix.shape[0]))
                

    #         else: 
    #             P_out = torch.inverse(alpha**(-1) * output_space_cov + torch.eye(output_space_cov.shape[0]))
    #             #calculate projections 
            


    #         #finally, update dictionary
    #         self.covs_and_projs[weight_name][2]= P_in
    #         self.covs_and_projs[weight_name][3] = P_out
